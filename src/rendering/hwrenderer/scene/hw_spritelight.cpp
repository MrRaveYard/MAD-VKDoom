...
void HWDrawInfo::GetDynSpriteLight(AActor* self, sun_trace_cache_t* traceCache, double x, double y, double z, FSection* sec, int portalgroup, float* out, bool fullbright, subsector_t * subsector)
{
	FDynamicLight* light;
	float frac, lr, lg, lb;
	float radius;
...
void HWDrawInfo::GetDynSpriteLight(AActor *thing, particle_t *particle, sun_trace_cache_t * traceCache, float *out)
{
	if (thing)
	{
		GetDynSpriteLight(thing, &thing->StaticLightsTraceCache, thing->X(), thing->Y(), thing->Center(), thing->section, thing->Sector->PortalGroup, out, (thing->flags5 & MF5_BRIGHT), thing->subsector);
	}
	else if (particle)
	{
		GetDynSpriteLight(nullptr, traceCache, particle->Pos.X, particle->Pos.Y, particle->Pos.Z, particle->subsector->section, particle->subsector->sector->PortalGroup, out, (particle->flags & SPF_FULLBRIGHT), particle->subsector);
	}
}
...
#if 0 // BIG BEAUTIFUL MERGE pending
	if (gl_light_skipbspwalkthreshold >= actorradius && self && self->section)
	{
		TMap<FDynamicLight*, std::unique_ptr<FLightNode>>::Iterator it(level.lightlists.flat_dlist[self->section->Index()]);
		TMap<FDynamicLight*, std::unique_ptr<FLightNode>>::Pair* pair;
		auto subsector = self->section->subsectors;
		while (it.NextPair(pair))
